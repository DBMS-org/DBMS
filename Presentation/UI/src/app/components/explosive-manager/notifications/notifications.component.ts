...
export class ExplosiveManagerNotificationsComponent implements OnInit, OnDestroy {
  ...
  private loadNotifications() {
    ...
    const sub = this.notificationService.fetchNotifications(0, 50).subscribe({
      next: (notifications) => {
        ...
        console.log('🔔 Filtered notifications for Explosive Manager:', explosiveManagerNotifications);
  ...
  private isExplosiveManagerRelevantNotification(type: any): boolean {
    ...
    const isTransferRequest = !!(typeStr && (
      ...
      typeStr === 'TransferDeliveryConfirmed'
    ));
    ...
    return isTransferRequest || isSystemNotification;
  }
  ...
  applyFilter() {
    ...
    switch (filter) {
      ...
      case 'inventory-alerts':
        filtered = allNotifications.filter(n => {
          const typeStr = typeof n.type === 'string' ? n.type : NotificationType[n.type];
          return typeStr === 'TransferRequestUrgent' ||
                 typeStr === 'TransferDispatched' ||
                 typeStr === 'TransferCompleted' ||
                 typeStr === 'TransferDeliveryConfirmed';
        });
